...
Endpoint identity is a deterministic canonical representation of the runtime route pattern plus HTTP method. It is not a proof of general route-matching semantic equivalence. Textual and programmatic parameter policies retain separate provenance markers, so a textual `int` token is not claimed equivalent to `IntRouteConstraint` without the application's actual routing-policy configuration. Built-in textual constraint-token casing, `length(3)` versus `length(3,3)`, HTTP-method constraint casing/order/duplicates, and composite child order are normalized. Inline regex commas and literal `;options=` remain pattern text. Supported programmatic regex requires the framework inline defaults; other options fail closed with `unsupported-parameter-policy`. Other representation details, including constraint arguments, defaults, catch-all encoding, optionality, regex text/options, and unsupported policy forms, remain identity-significant or fail closed. For example, `regex(^\\d+$)` and `regex(^\\D+$)` are different identities. The readable record may retain original casing. Equivalent bounded identities fail with `duplicate-endpoint-identity`, while representations outside the bounded rules may remain distinct. Programmatically constructed patterns without `RawText` use explicit provenance markers for catch-alls and supported parameter-policy objects; an unrepresentable parameter policy fails with `unsupported-parameter-policy` instead of being omitted. Encoded-slash catch-alls render as `{*name}` and non-encoded catch-alls as `{**name}`.
